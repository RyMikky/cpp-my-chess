...
bool DialineColision(std::vector<std::vector<Figure>>& deck,
	...
	if (x2 < x1 && y2 < y1) { // в первую четверть - x2 уменьшаетс€, y2 уменьшаетс€
		for (int x = x1 - 1, y = y1 - 1; x >= x2/*, y >= y2*/; x--, y--) {
			if (x == x2 && y == y2) {
				if (deck[x][y].color_ == FigureColor::grey || deck[x][y].color_ != deck[x1][y1].color_) {
					return true;
				}
				return false;
			}
	...
	if (x1 != x2 && y1 == y2) {
		if (x1 < x2) { // движение вниз
			for (int x = x1 + 1; x <= x2; x++) {
				if (x == x2) {
					if (deck[x][y1].color_ == FigureColor::grey || deck[x][y1].color_ != deck[x1][y1].color_) {
						return true;
					}
					return false;
				}
		...
		if (y1 < y2) { // движение вправо
			for (int y = y1 + 1; y <= x2; y++) {
				if (y == y2) {
					if (deck[x1][y].color_ == FigureColor::grey || deck[x1][y].color_ != deck[x1][y1].color_) {
						return true;
					}
					return false;
				}
	...
	if (color == FigureColor::white) {
		if (y1 == y2) {
			if (x1 == 6 && x2 == 4 // ѕервый ход
				&& deck[x1 - 1][y2].type_ == FigureType::zero
				&& deck[x2][y2].type_ == FigureType::zero) { 
				return true;
			} 
			...
			if (deck[x2][y2].color_ == FigureColor::black) {
				return true;
			} 
			else {
				return false;
		...
		if (y1 == y2) {
			...
			if (deck[x2][y2].color_ == FigureColor::white) {
				return true;
			}
			else {
				return false;

...
void GlobalTest() {
	...
	system("cls");
	{
		...
		for (int x = 0; x < 8; x++) {
			for (int y = 0; y < 8; y++) {
				assert(deck_.GetCurrentPosition(x, y).type_ == BaseFigureType::zero);
				Sleep(rand() % 20);
				std::cerr << ".";
			}
		}
...

	{
		...
		BaseFigure queen(1, "Ш", "Ферзь", BaseFigureType::queen, BaseFigureColor::black, true);
		std::cerr << queen.name_;

		deck_.FigureAdd(queen, 2, 1);
		std::cerr << "} по коордиатам B6";
		assert(deck_.GetCurrentPosition(2, 1).type_ == BaseFigureType::queen);

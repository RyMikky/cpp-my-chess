...
void ChessGame::GetPlayerTotalMoveCount(BaseFigureColor color) {
	if (BaseFigureColor::white == color) {
		std::cout << "Игрок белыми сделал суммарно " << total_white_player_moves_count_ << " ходов" << std::endl;
	}
	else if (BaseFigureColor::black == color) {
...
void ChessGame::PlayerMove(std::string label, BaseFigureColor color, const int& x1, const int& y1, const int& x2, const int& y2) {
	BaseFigure set = deck_.GetCurrentPosition(x1, y1);
	bool move;
	if (label == set.label_) {
	...
	if (move) {
		color == BaseFigureColor::white ? total_white_player_moves_count_++ : total_black_player_moves_count_++;
	}
